...
const Floor = () => {
  ...
  const fetchBhkList = async () => {
    const response = await fetch('/api/others/floor',{method:"GET"});
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await fetch('/api/others/floor', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'add', floor }),
    });
    setFloor('');
    fetchBhkList();
    toast.success('Floor type added successfully!', {
  ...
  const handleEdit = async (id) => {
    await fetch('/api/others/floor', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'edit', floorId: id, floor: floorValue }),
    });
    setFloorId(null);
    fetchBhkList();
    toast.success('Floor type updated successfully!', {
  ...
  const handleDelete = async (id) => {
    await fetch('/api/others/floor', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'delete', floorId: id }),
    });
    fetchBhkList();
